...
export default function OpenSourceContributionsCard() {
  ...
  return (
    <motion.div
      ...
      className="rounded-lg bg-white dark:bg-white/[0.04] p-3 sm:p-6"
    >
      {loading ? (
        <div className="space-y-4">
          {[1, 2, 3].map((i) => (
            <div key={i} className="animate-pulse">
              ...
              {i < 3 && <div className="mt-4 border-b border-neutral-300 dark:border-white/[0.08]" />}
            </div>
          ))}
        </div>
      ) : (
          ...
          {displayedContributions.map((contribution, index) => (
            ...
            >
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Link
                      ...
                      className="shrink-0 p-2 rounded-lg bg-neutral-200 border-2 border-neutral-500 dark:bg-white/[0.08] dark:border-white/[0.15] transition-all duration-300 group-has-hover:opacity-40 group-has-hover:group-hover/item:opacity-100"
                    >
              ...
              {index < displayedContributions.length - 1 && (
                <div className="mt-4 border-b border-neutral-300 dark:border-white/[0.08] transition-opacity duration-300 group-has-hover:opacity-40 group-has-hover:group-hover/item:opacity-100" />
              )}
            </motion.div>
          ))}
        </div>
      ...
      {!loading && contributions.length > 3 && (
        <div className="mt-4 pt-4 border-t border-neutral-300 dark:border-white/[0.08]">
          <button
